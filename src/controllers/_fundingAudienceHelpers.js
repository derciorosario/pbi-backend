...
async function setFundingAudience(funding, { identityIds, categoryIds, subcategoryIds, subsubCategoryIds }) {
  ...
  if (identityIds)        await funding.setAudienceIdentities(identityIds);
  if (categoryIds)        await funding.setAudienceCategories(categoryIds);
  if (subcategoryIds)     await funding.setAudienceSubcategories(subcategoryIds);
  if (subsubCategoryIds)  await funding.setAudienceSubsubs(subsubCategoryIds);
}
