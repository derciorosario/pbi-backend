...
async function setServiceAudience(service, { identityIds, categoryIds, subcategoryIds, subsubCategoryIds }) {
  ...
  if (identityIds)        await service.setAudienceIdentities(identityIds);
  if (categoryIds)        await service.setAudienceCategories(categoryIds);
  if (subcategoryIds)     await service.setAudienceSubcategories(subcategoryIds);
  if (subsubCategoryIds)  await service.setAudienceSubsubs(subsubCategoryIds);
}
