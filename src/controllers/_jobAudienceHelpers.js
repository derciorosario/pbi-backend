...
async function setJobAudience(job, { identityIds, categoryIds, subcategoryIds, subsubCategoryIds }) {
  ...
  if (identityIds)        await job.setAudienceIdentities(identityIds);
  if (categoryIds)        await job.setAudienceCategories(categoryIds);
  if (subcategoryIds)     await job.setAudienceSubcategories(subcategoryIds);
  if (subsubCategoryIds)  await job.setAudienceSubsubs(subsubCategoryIds);
}
