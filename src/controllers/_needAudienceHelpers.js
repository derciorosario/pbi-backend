...
async function setNeedAudience(need, { identityIds, categoryIds, subcategoryIds, subsubCategoryIds }) {
  ...
  if (identityIds)        await need.setAudienceIdentities(identityIds);
  if (categoryIds)        await need.setAudienceCategories(categoryIds);
  if (subcategoryIds)     await need.setAudienceSubcategories(subcategoryIds);
  if (subsubCategoryIds)  await need.setAudienceSubsubs(subsubCategoryIds);
}
