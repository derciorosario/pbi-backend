...
async function setProductAudience(product, { identityIds, categoryIds, subcategoryIds, subsubCategoryIds }) {
  ...
  if (identityIds)        await product.setAudienceIdentities(identityIds);
  if (categoryIds)        await product.setAudienceCategories(categoryIds);
  if (subcategoryIds)     await product.setAudienceSubcategories(subcategoryIds);
  if (subsubCategoryIds)  await product.setAudienceSubsubs(subsubCategoryIds);
}
