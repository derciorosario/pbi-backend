...
async function sendEmail(options) {
  try {
    ...
    await transporter.sendMail({
      from: `"55Links" <${process.env.EMAIL_FROM}>`,
      to,
      subject,
      html
    });
  ...
  try {
    ...
    const connections = await Connection.findAll({
      where: {
        ...
        // Connection model doesn't have a status field
      }
    });
    ...
    // Get recent posts from connected users
    const [events, services, products, tourismPosts] = await Promise.all([
      Event.findAll({
        where: {
          createdById: { [Op.in]: connectedUserIds },
    ...
    const updates = [
      ...events.map(event => ({ 
        ...
        link: `${process.env.FRONTEND_URL || 'https://panafricanbi.vercel.app'}/events/${event.id}`
      })),
      ...services.map(service => ({ 
        ...
        link: `${process.env.FRONTEND_URL || 'https://panafricanbi.vercel.app'}/services/${service.id}`
      })),
      ...products.map(product => ({ 
        ...
        link: `${process.env.FRONTEND_URL || 'https://panafricanbi.vercel.app'}/products/${product.id}`
      })),
      ...tourismPosts.map(post => ({ 
        ...
        link: `${process.env.FRONTEND_URL || 'https://panafricanbi.vercel.app'}/tourism/${post.id}`
      }))
    ];
  ...
  try {
    ...
    const scoredUsers = recommendedUsers.map(recommendedUser => {
      ...
      return {
        ...
        link: `${process.env.FRONTEND_URL || 'https://panafricanbi.vercel.app'}/profile/${recommendedUser.id}`,
        matchPercentage
      };
    });
